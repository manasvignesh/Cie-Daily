...
export function validateArticle(article: Pick<Article,'quick_brief'|'full_article'>): ValidationIssue[] {
  const q=article.quick_brief, f=article.full_article, out:ValidationIssue[]=[];
  if(!q.category.trim()) out.push({level:'error',path:'quick_brief.category',message:'Category is required.'});
  ...
  if(q.three_things_to_know.filter(x=>words(x)>=4).length<3) out.push({level:'error',path:'quick_brief.three_things_to_know',message:'Add three substantive points.'});
  ...
  const sections=f.explore_sections.filter(s=>words(`${s.content} ${s.items.map(i=>i.description).join(' ')}`)>=25);
  ...
  if(f.takeaways.filter(x=>words(x)>=4).length<3) out.push({level:'error',path:'full_article.takeaways',message:'Add at least three final takeaways.'});
  const fullWords=words([f.hook,f.what_happened,f.why_this_matters,f.bigger_picture,...f.explore_sections.map(s=>s.content),...f.takeaways].join(' '));
